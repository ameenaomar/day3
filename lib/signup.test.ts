import { describe, expect, it } from "vitest";
import {
  formatKuwaitMobile,
  kuwaitMobileDigits,
  parseSignUp,
  signInFromFormData,
  signUpFromFormData,
  toE164,
} from "@/lib/signup";

describe("kuwaitMobileDigits", () => {
  it("accepts the shapes people actually type", () => {
    for (const raw of ["99887766", "9988 7766", "+965 9988 7766", "+96599887766", "0096599887766", "965-99887766"]) {
      expect(kuwaitMobileDigits(raw)).toBe("99887766");
    }
  });

  it("accepts every valid Kuwaiti mobile prefix", () => {
    expect(kuwaitMobileDigits("50001234")).toBe("50001234");
    expect(kuwaitMobileDigits("60001234")).toBe("60001234");
    expect(kuwaitMobileDigits("90001234")).toBe("90001234");
  });

  it("rejects landlines, wrong lengths and other countries", () => {
    for (const raw of ["22334455", "1234567", "999887766", "", "abc", "+971501234567"]) {
      expect(kuwaitMobileDigits(raw)).toBeNull();
    }
  });

  it("keeps 965 when it is the number itself, not a country code", () => {
    // Eight digits starting 9 — a real mobile that happens to begin 965.
    expect(kuwaitMobileDigits("96512345")).toBe("96512345");
  });
});

describe("formatKuwaitMobile", () => {
  it("groups four and four, and never exceeds eight digits", () => {
    expect(formatKuwaitMobile("9988")).toBe("9988");
    expect(formatKuwaitMobile("99887766")).toBe("9988 7766");
    expect(formatKuwaitMobile("9988776612345")).toBe("9988 7766");
    expect(formatKuwaitMobile("abc9988x7766")).toBe("9988 7766");
  });
});

const passwords = { password: "correct-horse-8", passwordConfirm: "correct-horse-8" };

describe("parseSignUp", () => {
  const valid = {
    name: "Ameena Omar",
    email: "Ameena@Example.com",
    phone: "9988 7766",
    password: "correct-horse-8",
    passwordConfirm: "correct-horse-8",
    marketingOptIn: true,
    locale: "ar",
  };

  it("normalises what it accepts", () => {
    const result = parseSignUp(valid);
    expect(result.ok).toBe(true);
    if (!result.ok) return;
    expect(result.values.name).toBe("Ameena Omar");
    expect(result.values.email).toBe("ameena@example.com");
    expect(result.values.phone).toBe(toE164("99887766"));
    expect(result.values.marketingOptIn).toBe(true);
    expect(result.values.locale).toBe("ar");
  });

  it("treats the phone as optional", () => {
    const result = parseSignUp({ ...valid, phone: "" });
    expect(result.ok).toBe(true);
    if (!result.ok) return;
    expect(result.values.phone).toBeNull();
  });

  it("defaults marketing consent to off", () => {
    const result = parseSignUp({
      name: "Ameena Omar",
      email: "a@b.com",
      password: "correct-horse-8",
      passwordConfirm: "correct-horse-8",
    });
    expect(result.ok).toBe(true);
    if (!result.ok) return;
    expect(result.values.marketingOptIn).toBe(false);
    expect(result.values.locale).toBe("en");
  });

  it("reports a missing name and a bad email by code", () => {
    const result = parseSignUp({ name: "", email: "not-an-email", ...passwords });
    expect(result.ok).toBe(false);
    if (result.ok) return;
    expect(result.errors.name).toBe("name_required");
    expect(result.errors.email).toBe("email_invalid");
  });

  it("rejects a one-letter name and a non-Kuwaiti phone", () => {
    const result = parseSignUp({ name: "A", email: "a@b.com", phone: "22334455", ...passwords });
    expect(result.ok).toBe(false);
    if (result.ok) return;
    expect(result.errors.name).toBe("name_short");
    expect(result.errors.phone).toBe("phone_invalid");
  });

  it("collapses whitespace inside a name", () => {
    const result = parseSignUp({ ...valid, name: "  Noura \t  Al-Sabah  " });
    expect(result.ok).toBe(true);
    if (!result.ok) return;
    expect(result.values.name).toBe("Noura Al-Sabah");
  });

  it("requires eight characters", () => {
    const result = parseSignUp({ ...valid, password: "short7!", passwordConfirm: "short7!" });
    expect(result.ok).toBe(false);
    if (result.ok) return;
    expect(result.errors.password).toBe("password_short");
  });

  it("requires a password at all", () => {
    const result = parseSignUp({ ...valid, password: "", passwordConfirm: "" });
    expect(result.ok).toBe(false);
    if (result.ok) return;
    expect(result.errors.password).toBe("password_required");
  });

  it("refuses one longer than Supabase stores", () => {
    const long = "x".repeat(73);
    const result = parseSignUp({ ...valid, password: long, passwordConfirm: long });
    expect(result.ok).toBe(false);
    if (result.ok) return;
    expect(result.errors.password).toBe("password_long");
  });

  it("requires the confirmation to match", () => {
    const result = parseSignUp({ ...valid, passwordConfirm: "correct-horse-9" });
    expect(result.ok).toBe(false);
    if (result.ok) return;
    expect(result.errors.passwordConfirm).toBe("password_mismatch");
  });

  it("reports the length before the mismatch, so one message shows at a time", () => {
    const result = parseSignUp({ ...valid, password: "short", passwordConfirm: "different" });
    expect(result.ok).toBe(false);
    if (result.ok) return;
    expect(result.errors.password).toBe("password_short");
    expect(result.errors.passwordConfirm).toBeUndefined();
  });

  it("gives one error per field", () => {
    const result = parseSignUp({ name: "", email: "", ...passwords });
    expect(result.ok).toBe(false);
    if (result.ok) return;
    expect(Object.keys(result.errors).sort()).toEqual(["email", "name"]);
  });
});

describe("signUpFromFormData", () => {
  it("reads a plain form post, unchecked checkbox and all", () => {
    const form = new FormData();
    form.set("name", "  Yousef Al Ali  ");
    form.set("email", " YOUSEF@example.com ");
    form.set("password", "correct-horse-8");
    form.set("passwordConfirm", "correct-horse-8");
    form.set("locale", "en");
    const result = signUpFromFormData(form);
    expect(result.ok).toBe(true);
    if (!result.ok) return;
    expect(result.values.name).toBe("Yousef Al Ali");
    expect(result.values.email).toBe("yousef@example.com");
    expect(result.values.marketingOptIn).toBe(false);
    expect(result.values.phone).toBeNull();
  });

  it("reads a ticked consent box", () => {
    const form = new FormData();
    form.set("name", "Ameena Omar");
    form.set("email", "a@b.com");
    form.set("password", "correct-horse-8");
    form.set("passwordConfirm", "correct-horse-8");
    form.set("marketingOptIn", "on");
    const result = signUpFromFormData(form);
    expect(result.ok).toBe(true);
    if (!result.ok) return;
    expect(result.values.marketingOptIn).toBe(true);
  });
});

describe("signInFromFormData", () => {
  it("normalises the email and keeps the password verbatim", () => {
    const form = new FormData();
    form.set("email", "  Ameena@Example.com ");
    form.set("password", "  spaces matter  ");
    const result = signInFromFormData(form);
    expect(result.ok).toBe(true);
    if (!result.ok) return;
    expect(result.values.email).toBe("ameena@example.com");
    expect(result.values.password).toBe("  spaces matter  ");
  });

  it("does not judge the length of an existing password", () => {
    // An account older than the current minimum must still be able to sign in.
    const form = new FormData();
    form.set("email", "a@b.com");
    form.set("password", "abc");
    expect(signInFromFormData(form).ok).toBe(true);
  });

  it("reports an empty password and a bad email by code", () => {
    const form = new FormData();
    form.set("email", "nope");
    form.set("password", "");
    const result = signInFromFormData(form);
    expect(result.ok).toBe(false);
    if (result.ok) return;
    expect(result.errors.email).toBe("email_invalid");
    expect(result.errors.password).toBe("password_required");
  });
});
